...
//    User getOne(long id);
@Query(value ="SELECT * FROM users join ratings on users.id = ratings.recipient_user_id join user_interest u on users.id = u.user_id join interests on u.interest_id = interests.id where users.first_name LIKE :myInput or users.last_name LIKE :myInput or location LIKE :myInput or interests.name like :myInput", nativeQuery = true)
